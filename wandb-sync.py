...
def parse_args():
    ...
    parser.add_argument(
        ...
        "--n_workers",
        type=int,
        help="Number of workers to use. If not specified, will use all available cores. (Recommended: n_cpus*3)",
    )
    parser.add_argument(
        "-l", "--limit", type=int, help="Limit the number of runs to sync"
    )
    parser.add_argument(
        "-r",
        "--reverse",
        action="store_true",
        help="Reverse the order of runs to sync?",
    )
    parser.add_argument(
        "-c", "--clean_after", action="store_true", help="Clean run after syncing?"
    )
    args = parser.parse_args()

    if args.n_workers is None:
        args.n_workers = (
            4 if not hasattr(os, "sched_getaffinity") else len(os.sched_getaffinity(0))
        )
...
if __name__ == "__main__":
    ...
    if check_offline():
        ...
        runlist = [
            os.path.join("wandb", f)
            for f in os.listdir("wandb")
            if "offline" in f.lower()
        ]
