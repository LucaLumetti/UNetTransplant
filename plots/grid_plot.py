...
tasks = ['Mandible+Canals_Naive', 'Pharynx+Teeth_Naive', 'Kidney+Liver_Sharp', 'Spleen+Stomach_Sharp',
         'Mandible+Canals_Stable1', 'Pharynx+Teeth_Stable1', 'Kidney+Liver_Stable', 'Spleen+Stomach_Stable', ]
...
for ax, title in zip(axs, tasks):
    ...
    if i<8:
        data_im = ax.imshow(half_data, extent=[0, 1, 0, 1], origin='lower', cmap=cmap, interpolation='bicubic', vmax=1.0)
        # Overlay fewer contour lines focusing on levels from 0.40 to 0.80 with increased gaps
        contour = ax.contour(np.linspace(0, 1, half_data.shape[0]), np.linspace(0, 1, half_data.shape[1]), 
                            half_data, levels=np.linspace(0.4, 0.8, 5), colors='black', linewidths=0.7, alpha=0.7)
    else:
        data_im = ax.imshow(half_data, extent=[0, 2, 0, 2], origin='lower', cmap=cmap, interpolation='bicubic', vmax=1.0)    # Overlay fewer contour lines focusing on levels from 0.40 to 0.80 with increased gaps
        contour = ax.contour(np.linspace(0, 2, half_data.shape[0]), np.linspace(0, 2, half_data.shape[1]), 
                         half_data, levels=np.linspace(0.4, 0.8, 5), colors='black', linewidths=0.7, alpha=0.7)
    ...
    if i<4:
        max_x, max_y = max_indices[1] / 20, max_indices[0] / 20  # convert indices to scale based on your plot's extent
    elif i<8 and 'PrevisioniMeteoFlatAvgToothFairy/' in data_path:
        max_x, max_y = max_indices[1] / 12, max_indices[0] / 12  # convert indices to scale based on your plot's extent
    elif i<8:
        max_x, max_y = max_indices[1] / 12, max_indices[0] / 12  # convert indices to scale based on your plot's extent
    else:
    ...
    ax.plot(max_x, max_y, '*', markersize=12, markeredgewidth=1, color='red', markeredgecolor='black', label='Best Value')
    ...
    if i<8:
        ax.plot(0.5, 0.5, 'D', markersize=6, markeredgewidth=1, label='Default', markeredgecolor='black', color='white')
    else:
        ax.plot(1, 1, 'D', markersize=5, markeredgewidth=1, label='Default', markeredgecolor='black', color='cyan')
...
fig.text(0.080, 0.69, 'Plastic - Avg', va='center', rotation='vertical', fontsize=12, weight='bold')
fig.text(0.076, 0.30, 'Stable - Avg', va='center', rotation='vertical', fontsize=12, weight='bold')

fig.text(0.25, 0.953, 'ToothFairy2', va='center', rotation='horizontal', fontsize=12, weight='bold')
fig.lines.extend([plt.Line2D([0.125, 0.472], [0.93, 0.93], transform=fig.transFigure, color="black", linestyle="-")])

fig.text(0.62, 0.953, 'BTCV Abdomen', va='center', rotation='horizontal', fontsize=12, weight='bold')
fig.lines.extend([plt.Line2D([0.503, 0.85], [0.93, 0.93], transform=fig.transFigure, color="black", linestyle="-")])
...
cbar_ax = fig.add_axes([0.87, 0.125, 0.015, 0.74])  # Adjust the colorbar position vertically.
...
axs[-5].legend(loc='upper right', fancybox=False, edgecolor='black')#, frameon=True, facecolor='white', framealpha=1)
