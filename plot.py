...
for folder in os.listdir('./output_UNetMergingmerge'):
    ...
    half_data = data[21:, 21:]  # Adjust indexing as necessary based on your specific data layout
    ...
    data_im_reduced_contours_star = plt.imshow(half_data, extent=[0, 2, 0, 2], origin='lower', cmap=cmap, interpolation='bicubic', vmax=1.0)
    ...
    contour_spaced_star = plt.contour(np.linspace(0, 2, half_data.shape[1]), np.linspace(0, 2, half_data.shape[0]), 
                                    half_data, levels=np.linspace(0.4, 0.9, 6), colors='black', linewidths=0.7, alpha=0.7)
    ...
    max_x, max_y = max_indices[1] / 10, max_indices[0] / 10  # convert indices to scale based on your plot's extent

    # Mark the maximum value with a red star
    plt.plot(max_x, max_y, '*', markersize=12, markeredgewidth=1, color='red', markeredgecolor='black')

    #plot marker at x=1 and y=1
    plt.plot(1, 1, '+', markersize=12, markeredgewidth=1, color='green', markeredgecolor='black')
    ...
    plt.text(max_x, max_y+0.015, f'{max_value:.2f}', fontsize=10, ha= 'center', va='bottom', color='black')
