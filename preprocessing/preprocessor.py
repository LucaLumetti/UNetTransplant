...
class Preprocessor:
    ...
    def preprocess(
        ...
        label_array = sitk.GetArrayFromImage(label).astype(np.int16, copy=False)
    ...
    def run(
        ...
        for sample in tqdm(self.dataset.train_samples, desc=f"Preprocessing {self.dataset.name}"):
            ...
            for i, (image_patch, label_patch) in enumerate(patches):
                if np.random.rand() > keep_empty_prob and np.sum(label_patch) == 0:
                    continue
