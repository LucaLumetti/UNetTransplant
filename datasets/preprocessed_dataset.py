...
class PreprocessedDataset(torch.utils.data.Dataset):
    ...
    def load_subjects(self,) -> List:
        ...
        for volume_dir in all_volumes_dir:
            images_path = (preprocessed_dataset_path / volume_dir / 'images').glob('*.npy')
            labels_path = (preprocessed_dataset_path / volume_dir / 'labels').glob('*.npy')
            subjects.append({
                'images': list(images_path),
                'labels': list(labels_path)
            })
        return subjects
